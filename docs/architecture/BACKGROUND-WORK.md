...
| Hardware page refresh | `HardwareViewModel.cs` | configured 1–10 s | User enables monitor / disables or disposes page | Lease-backed metrics; LibreHardwareMonitor `Update()` and RAM fallback on cache miss | Native/hardware; opt-in; shares one backend and 200 ms runtime snapshot |
| Companion snapshot publisher | `AppTelemetrySnapshotProvider.cs` | 500 ms | Companion server starts / stops | Reads hardware only when a hardware lease exists; combines Active Game and live performance | Medium with connected hardware consumer; no network itself |
| Telemetry WebSocket sender | `TelemetryWebSocketHandler.cs` | 500 ms | Authenticated WS connected / disconnect/cancel | Sends current snapshot; owns a hardware lease while connected | Network + hardware activation; shares published snapshot/backend |
...
| Companion Session poll | `session-optimization.js` | 4 s | Frontend init / page unload | `GET /api/v1/session-optimization` with in-flight guard | Network; unchanged |
...
- Companion connected: Active Game (2 s), published snapshot (500 ms), live PresentMon (250 ms), and WS send (500 ms) run; hardware activates through the connection lease. Frontend REST telemetry polling stops when WS opens.
