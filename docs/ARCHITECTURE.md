...
The WPF shell uses bundled Sora static fonts for interface text and bundled Space Grotesk static fonts for large performance metrics. Fonts, colors, spacing, radii, and control templates are centralized in the application theme dictionaries so packaged builds do not depend on fonts installed in Windows. Both font families are distributed under the SIL Open Font License 1.1; exact notices are listed in `docs/THIRD-PARTY-NOTICES.md` and installed with the application.
