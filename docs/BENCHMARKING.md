...
### Environment metadata (v1)

`session.json` may carry an optional `environment` object: a one-shot, immutable, best-effort snapshot of the capture context (OS description/build, CPU name, GPU name/driver version, total RAM, primary display resolution/refresh rate, plus the FrameHub version already stored in session metadata). It is captured exactly once per benchmark by `BenchmarkCaptureCoordinator` through `IBenchmarkEnvironmentProvider` immediately before session creation, is never updated during capture, and uses only safe one-shot OS queries (WMI metadata, `GlobalMemoryStatusEx`, `EnumDisplaySettings`). It does not use or enable Hardware Monitor, LibreHardwareMonitor, PawnIO, or elevation, and it contains no personally identifying information. Individual unavailable fields stay null; provider failure is logged and never aborts capture. Historical schema-v1 records load unchanged — the previous empty `environment` placeholder deserializes with all fields unavailable, no file rewrite or schema bump is required, and additive optional fields keep schema version 1.
